...
def get_ensemble_prediction(models_configs, data, gold_model):
    docs = {}
    doc_names = []
    model_predictions = {}
    ...
    for model_ind, model_config in enumerate(models_configs):
        ...
        for batch in data:
            if gold_model:
                ...
                sentences, speakers, doc_key = batch[0]
                if doc_key not in doc_names:
                    doc_names.append(doc_key)
                top_span_starts, top_span_ends, top_antecedents, top_antecedent_scores = \
                    model.predict_for_ensemble(sentences, speakers, doc_key)
    ...
    ensemble_predicted_clusters, mention_to_predicted = mean_prediction(docs, num_models)
