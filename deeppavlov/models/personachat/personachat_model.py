...
@register('personachat_simple_model')
class PersonaChatSimpleModel(TFModel):
    def __init__(self, **kwargs):
        self.opt = deepcopy(kwargs)
        self.init_word_emb = self.opt['word_emb']
        self.init_char_emb = self.opt['char_emb']
    ...
    def _init_graph(self):
        ...
        self.persona_c = tf.slice(self.persona_c_ph, [0, 0, 0], [bs, self.persona_maxlen, self.char_limit])
        self.utt_c_len = tf.reshape(tf.reduce_sum(tf.cast(tf.cast(self.utt_c, tf.bool), tf.int32), axis=2), [-1])
        self.persona_c_len = tf.reshape(tf.reduce_sum(tf.cast(tf.cast(self.persona_c, tf.bool), tf.int32), axis=2),
                                        [-1])
        ...
        with tf.variable_scope('decoder'):
            ...
            decoder_cell_size = self.hidden_size
            cell_state = tf.layers.dense(cell_state, decoder_cell_size,
                                         kernel_initializer=tf.contrib.layers.xavier_initializer(), )
            if self.cell_type == 'GRU':
                cell_state = (cell_state, cell_state)
                cell_out = cell_state[0]
            ...
            persona_do = tf.nn.dropout(persona, keep_prob=self.keep_prob, noise_shape=[bs, 1, tf.shape(persona)[-1]])
            utt_do = tf.nn.dropout(utt, keep_prob=self.keep_prob, noise_shape=[bs, 1, tf.shape(utt)[-1]])
            dropout_mask = tf.nn.dropout(tf.ones([bs, tf.shape(cell_state[-1])[-1]], dtype=tf.float32),
                                         keep_prob=self.keep_prob)
            token_dropout_mask = tf.nn.dropout(tf.ones([bs, self.word_emb_dim], dtype=tf.float32),
                                               keep_prob=self.keep_prob)
            output_token = tf.zeros(shape=(bs,), dtype=tf.int32)
            pred_tokens = []
            logits = []
            probs = []
            for i in range(self.y_len_limit):
                inp_persona, _ = attention(persona_do, cell_out * dropout_mask, self.hidden_size, self.persona_mask,
                                         scope='att_match')
                ...
                input = tf.concat([input_token_emb, inp_persona], axis=-1)
