...
class PersonaChatModel(TFModel):
    ...
    def _init_graph(self):
        ...
        with tf.variable_scope('decoder'):
            ...
            state = tf.layers.dense(state, self.hidden_size)
            ...
            for i in range(self.seq_len_limit):
                ...
                print('inp_match:',inp_match)
                print('inp_utt:',inp_utt)
                ...
                output_token = tf.arg_max(output_probs, dimension=-1)
                print('output_token:', output_token)
                pred_tokens.append(output_token)
            self.yp = tf.transpose(tf.stack(pred_tokens, axis=0), [1, 0])
            print('yp:', self.yp)
            logits = tf.transpose(tf.stack(logits, axis=0), [1, 0, 2])
            print('logits:', logits)
            probs = tf.transpose(tf.stack(probs, axis=0), [1, 0, 2])
            print('probs:', probs)

        self.y = tf.one_hot(self.y, depth=self.vocab_size)
        self.loss = tf.nn.softmax_cross_entropy_with_logits(labels=self.y, logits=logits) * self.y_mask
    ...
    def __call__(self, persona_tokens, persona_chars, utt_tokens, utt_chars, *args, **kwargs):
        ...
        yp = self.sess.run(self.yp, feed_dict=feed_dict)
        return yp
