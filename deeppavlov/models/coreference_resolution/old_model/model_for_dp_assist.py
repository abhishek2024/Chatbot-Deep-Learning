...
class CorefModel(TFModel):
    ...
    def __call__(self, model_batch: List[List[List[str]]], **kwargs) -> List[List[Tuple]]:
        model_outputs = []
        for i, sentences in enumerate(model_batch):
            model_outputs.append(self.predict(sentences,
                                              [["spk_1" for token in sent] for sent in sentences],
                                              f"doc_{i}",
                                              []))
