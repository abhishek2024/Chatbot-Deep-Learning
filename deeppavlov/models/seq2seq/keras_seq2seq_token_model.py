...
class KerasSeq2SeqTokenModel(KerasClassificationModel):
    ...
    def encoder_decoder_model(self, **kwargs) -> Model:
        ...
        if isinstance(self._decoder_input_state, list):
            self.decoder_model = Model(inputs=[self._decoder_infer_emb_inp] + self._decoder_input_state,
    ...
    def infer_on_batch(self, x: List[List[np.ndarray]], **kwargs) -> List[List[int]]:
        ...
        for i in range(len(x)):  # batch size
            ...
            while not end_of_sequence:
                output = self.decoder_model.predict([np.array([[current_token]])] + state)
                token_probas = output[0]
                state = output[1:]
