...
def dot_attention_relation(inputs, memory, mask, att_size, keep_prob=1.0,
                  use_gate=False, drop_diag=False, use_transpose_att=False, inputs_mask=None,
                  scope="dot_attention_rel", reuse=False):
    ...
    Args:
        inputs: Tensor [batch_size x input_len x feature_size]
        memory: Tensor [batch_size x memory_len x relation_size x feature_size] (relation matrix)
        mask: memory mask
        att_size: hidden size of attention
        keep_prob: dropout keep_prob
    ...
    Returns:
        attention vectors [batch_size x input_len x relation_size x (feature_size + feature_size)]
    """


    with tf.variable_scope(scope, reuse=reuse):
        BS, IL, IH = tf.unstack(tf.shape(inputs))
        BS, ML, MR, MH = tf.unstack(tf.shape(memory))

        d_inputs = tf.nn.dropout(inputs, keep_prob=keep_prob, noise_shape=[BS, 1, IH])
        d_memory = tf.nn.dropout(memory, keep_prob=keep_prob, noise_shape=[BS, 1, MR, MH])
        ...
        with tf.variable_scope("attention"):
            ...
            inputs_att = tf.expand_dims(inputs_att, axis=2) # to add relation dim

            #print("inputs_att", inputs_att)

            logits = tf.matmul(inputs_att, tf.transpose(memory_att, [0, 1, 3, 2])) / (att_size ** 0.5)

            #print("logits", logits)

            #logits = tf.tile(tf.expand_dims(mask, axis=2), [1, 1, MH, 1])
            ...
            att_weights = tf.cast(tf.nn.softmax(tf.cast(softmax_mask(logits, mask), tf.float64)), tf.float32)
...
def simple_attention(memory, att_size, mask, keep_prob=1.0, scope="simple_attention"):
    ...
    with tf.variable_scope(scope):
        ...
        att_weights = tf.expand_dims(tf.cast(tf.nn.softmax(tf.cast(logits, tf.float64)), tf.float32), axis=2)
        res = tf.reduce_sum(att_weights * memory, axis=1)
        return res

def simple_attention_rel(memory, att_size, keep_prob=1.0, scope="simple_attention_rel"):
    """Simple attention without any conditions.
    ...
    with tf.variable_scope(scope):
        #print("memory", memory)
        BS, ML, ML2, MH = tf.unstack(tf.shape(memory))
        memory_do = tf.nn.dropout(memory, keep_prob=keep_prob, noise_shape=[BS, 1, ML2, MH])
        logits = tf.layers.dense(tf.layers.dense(memory_do, att_size, activation=tf.nn.tanh), 1, use_bias=False)
        #print("logits", logits)
        
        squeeze = tf.squeeze(logits, [3])
        c_zeros= tf.zeros(tf.shape(squeeze), dtype = tf.float32)
        #print("squeeze", squeeze)
        
        mask = tf.equal(c_zeros, squeeze)

        logits = softmax_mask(squeeze, mask)
        #print("logits", logits)
        att_weights = tf.expand_dims(tf.cast(tf.nn.softmax(tf.cast(logits, tf.float64)), tf.float32), axis=3)
        #print("att_weights", att_weights)
        res = tf.reduce_sum(att_weights * memory, axis=2)
        #print("res", res)
        return res
