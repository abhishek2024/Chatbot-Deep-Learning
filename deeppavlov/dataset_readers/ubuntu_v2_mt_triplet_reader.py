...
@register('ubuntu_v2_mt_triplet_reader')
class UbuntuV2MTTripletReader(DatasetReader):
    ...
    def preprocess_data_train(self, train_fname: Union[Path, str]) -> List[Tuple[List[str], int]]:
        ...
        with open(train_fname, 'r') as f:
            ...
            for el in reader:
                if int(el[2]) == 1:
                    contresp[el[0]].append(el[1])
            for i, (c, r) in enumerate(contresp.items()):
                for el in r:
                    data.append((self._expand_context(c.split('__eot__')) + [el], i))
        return data
