...
import sys
sys.path.append('/home/gureenkova/projects/DeepPavlov')
...
file = logging.FileHandler('../eval_logs/ranker_tfidf_with_pop.log')
...
parser.add_argument("-dataset_path", help="path to SQuAD dataset", type=str,
                    default='/home/gureenkova/data/squad/dev-v1.1_prep_4ranker.json')
parser.add_argument("-ranker_answers_path", help="path to JSON with ranker answers", type=str,
                    default='/home/gureenkova/projects/DeepPavlov/deeppavlov/skills/odqa/eval_scripts/ranker_answers_tfidf_top_50.json')
parser.add_argument("-popularities_path", help="path to JSON with ranker answers", type=str,
                    default='/home/gureenkova/data/squad/popularities.json')
parser.add_argument("-classifier_path", help="path to JSON with ranker answers", type=str,
                    default='/home/gureenkova/projects/DeepPavlov/download/odqa/rf_pop.joblib')
# parser.add_argument("-scaler_path", help="path to JSON with ranker answers", type=str,
#                     default='/media/olga/Data/projects/iPavlov/DeepPavlov/download/odqa/scaler_pop.joblib')
...
def main():
    ...
    #popularities = read_json(args.popularities_path)
    ...
    try:
        ...
        # Re-ranking according to popularity:
        # TODO Check if tfidf values is not different for different units (article, chunks) of ranking

        #for ra_list in ranker_answers:
            #for ra in ra_list:
                #pop = popularities[ra[TITLE_IDX]]
                #ra.append(pop)
                #tfidf_score = ra[TFIDF_SCORE_IDX]
                #prob = clf.predict_proba([[tfidf_score, pop]])
                #ra.append(prob[0][1])

        #del popularities
        ranker_answers = read_json('ranker_answers_tfidf_top_50_with_pop_unsorted.json')

        for l in range(len(ranker_answers)):
            ranker_answers[l] = sorted(ranker_answers[l], key=itemgetter(5, 2), reverse=True)
        save_json(ranker_answers, 'ranker_answers_tfidf_top_50_with_pop.json')
