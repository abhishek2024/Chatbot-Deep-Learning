...
file = logging.FileHandler('../eval_logs/odqa_ensemble+noans_chunks_true_stat.log')
file.setFormatter(fmt)
logger.addHandler(file)

parser = argparse.ArgumentParser()

parser.add_argument("-ranker_answers_path", help="path to a JSON ranker config", type=str,
                    default='/media/olga/Data/datasets/squad/ranker_answers_ensemble_top_50.json')
...
parser.add_argument("-reader_config_path", help="path to squad reader config", type=str,
                    default='../../../../deeppavlov/configs/squad/multi_squad_noans_chunks.json')
    ...
    try:
        ...
        for n in range(5, 51, 5):
            ...
            for qa, ranker_answer in zip(dataset, ranker_answers):
                ...
                preds = list(map(itemgetter(0), y_pred))
                truth = y_true[i][0]

                true_answers_stat += int(any(t for t in truth if t in preds))
                ...
                logger.info(
                    "Processed {} qa pairs for n={}".format(i+1, n))
                i += 1
            # print(y_true)
            # print(y_pred_on_i)
            ...
            logger.info(
                'True answers stat mapping for top {}: {}'.format(n, true_answers_stat / qa_dataset_size)
            )
            true_answers_stat_mapping[n] = true_answers_stat / qa_dataset_size
        ...
        logger.info(f"True answers stat mapping: {true_answers_stat_mapping}")
